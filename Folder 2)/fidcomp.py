...
class FidCompUnitary(FidelityComputer):
    ...
    ########################################################################################
    # Additional fidelity funcitons that allow the optimizer to prioritize low frequency pulses

    def get_hf_fid_err(self,amps_1,fmax):
        """
        Define a new method that characterizes how much high frequency components
        there are in the current pulse. This will be used as a penalty in the 
        get_fid_err method.
        """
        #create handle to access the optimizers dynamics instance
        dyn = self.parent
        #total number of control parameters in the Hamiltonian
        n_ctrls = dyn.num_ctrls
        #number of time steps in the pulse
        n_ts = dyn.num_tslots
        #Total length of evolution
        T = dyn._evo_time
        # sampling rate
        sr=n_ts/T
        #sampling interval
        ts=1/sr
        # print(amps_1)
        # Calculate the list of frequencies for the rfft. We can again do this
        # just once becuase each pulse has the same length and time interval size.
        # It should be looked into later how to calcuate this only once over all
        # not each time the gradient function is called
        f=rfftfreq(n_ts,ts)
        # Create weighting function.
        weight1 = self.w1(f,fmax)
        # loop through control parameters and within each parameter
        # loop through each piece of the pulse
        #Begin loop over controls
        for j in range(n_ctrls):
            # Initialize the fid err
            if j == 0:
                hf_fid_err=0
            #Up front, for each control pramater, calculate the FFT
            X=rfft(amps_1[:,j],norm="forward")
            for k in range(n_ts):
                for i in range(len(f)):
                    hf_fid_err += weight1[i]*np.conjugate(X[i])*X[i]
        return np.real(hf_fid_err)
    
    def get_hf_err_gradient(self,amps_1,fmax):
        """
        ...
        """
        #create instance of dynamics class to access pulse parameters
        dyn = self.parent
        #total number of control parameters in the Hamiltonian
        n_ctrls = dyn.num_ctrls
        #number of time steps in the pulse
        ...
        hfgrad = np.zeros([n_ts, n_ctrls], dtype=float)
        # Calculate the DFT matrix upfront. This only needs to be done once
        # because n_ts is the same for each pulse and so the size of the DFT
        # matrix is the same. The transpose requirement is strange and understood
        # only empiraclly. Look into this later
        dftmtx = np.transpose(rfft(np.eye(n_ts),norm="forward"))
        # Calculate the list of frequencies for the rfft. We can again do this
        # just once becuase each pulse has the same length and time interval size.
        # It should be looked into later how to calcuate this only once over all
        # not each time the gradient function is called
        ...
        for j in range(n_ctrls):
            ...
            for k in range(n_ts):
                for i in range(len(f)):
                    if i == 0:
                        sum=0
                    sum += weight1[i]*(np.conjugate(dftmtx[i][k])*X[i] + np.conjugate(X[i])*dftmtx[i][k])
                hfgrad[k, j] = np.real(sum)
        self.grad_norm = np.sqrt(np.sum(hfgrad**2))
        # update gradeint normal such to inform optimizer of convergance to specificed tolerance
    ...
    #########################################################################################
    # Below is the original source code fidelity function defintions

    def get_fid_err(self):
        """
        Gets the absolute error in the fidelity
        """
        # return np.real(self.get_hf_fid_err())
        # return np.abs(1 - self.get_fidelity()) + np.real(self.get_hf_fid_err())
    ...
    def get_fid_err_gradient(self):
        ...
        """
        # commenting out whole seciton here to see what happens if the optimizer just trys
        # work with the hf gradient
