...
class Optimizer(object):
    ...
    def _build_bounds_list(self):
        ...
        T=dyn._evo_time
        t_tot=dyn.num_tslots
        t0=T/t_tot
        #Envelope function for bounds is a double fermi function
        def e_func(t1,tf,rise_v,rise_t):
            C=2
            B=-C/2
            A=-rise_t/np.log(C/(rise_v-B)-1)
            return (C/(np.exp(-t1/A)+1)+B)*(-1*(C/(np.exp(-(t1-tf)/A)+1)+B))
        self.bounds = []
        #Current parameters for double fermi function means that bounds reach
        #their 95% maximum value .4ns away from the start and end of pulse.
        #Although this is not something that can be played with in
        #"pulse optimization w GRAPE", you can easily change the parameters here.
        for t in range(t_tot):
            if t < (t_tot-1):
                for c in range(n_ctrls):
                    if isinstance(self.amp_lbound, list):
                        lb = e_func(t0*t,T,.95,.4)*self.amp_lbound[c]
                    else:
                        lb = e_func(t0*t,T,.95,.4)*self.amp_lbound
                    if isinstance(self.amp_ubound, list):
                        ub = e_func(t0*t,T,.95,.4)*self.amp_ubound[c]
                    else:
                        ub = e_func(t0*t,T,.95,.4)*self.amp_ubound
                ...
                for c in range(n_ctrls):
                    if isinstance(self.amp_lbound, list):
                        lb = e_func(t0*(t+1),T,.95,.4)*self.amp_lbound[c]
                    else:
                        lb = e_func(t0*(t+1),T,.95,.4)*self.amp_lbound
                    if isinstance(self.amp_ubound, list):
                        ub = e_func(t0*(t+1),T,.95,.4)*self.amp_ubound[c]
                    else:
                        ub = e_func(t0*(t+1),T,.95,.4)*self.amp_ubound
    ...
    def run_optimization(self, term_conds=None):
        ...
        The optimisation ends when one of the passed termination conditions
    ...
    def fid_err_func_wrapper(self, *args):
        ...
        Although this code is calle fid_err_func_wrapper, it is a wrapper
        for the fidelity error and high frequency error.
        """
        ...
        tc = self.termination_conditions
        f_err = self.dynamics.fid_computer.get_fid_err()
        hf_err = self.dynamics.fid_computer.get_hf_fid_err(amps,self.hfc)
        tot_err = hf_err + f_err
        self.tot_err=tot_err
        self.hf_err=hf_err

        if self.num_fid_func_calls % 10 == 0:
            print("fid calls")
            print(self.num_fid_func_calls)
        ...
        if self.iter_summary:
            ...
            self.iter_summary.fid_err = tot_err

        if self.dump and self.dump.dump_fid_err:
            self.dump.update_fid_err_log(tot_err)

        if tot_err <= tc.fid_err_targ:
            raise errors.GoalAchievedTerminate(tot_err)
    ...
    def fid_err_grad_wrapper(self, *args):
        ...
        grad_1 = fid_comp.get_fid_err_gradient() + fid_comp.get_hf_err_gradient(amps,self.hfc)

        self.grad_norm=np.sqrt(np.sum(grad_1**2))
        if self.num_grad_func_calls % 10 == 0:
            print("grad calls")
            print(self.num_grad_func_calls)
            print("grad norm")
            print(self.grad_norm)
        if self.iter_summary:
            self.iter_summary.grad_func_call_num = self.num_grad_func_calls
            self.iter_summary.grad_norm = self.grad_norm

        if self.dump:
            if self.dump.dump_grad_norm:
                self.dump.update_grad_norm_log(self.grad_norm)

            if self.dump.dump_grad:
                self.dump.update_grad_log(self.grad)

        tc = self.termination_conditions
        if self.grad_norm < tc.min_gradient_norm:
            raise errors.GradMinReachedTerminate(self.grad_norm)
        return grad_1.flatten()
    ...
    def _add_common_result_attribs(self, result, st_time, end_time):
        ...
        result.hf_err=self.hf_err
        result.tot_err=self.tot_err
        result.grad_norm_final = self.grad_norm
    ...
    def run_optimization(self, term_conds=None):
        ...
        if self.log_level <= logging.INFO:
            ...
            if self.approx_grad:
                msg += " (approx grad)"
            logger.info(msg)
            print("optimizer min grad")
            print(term_conds.min_gradient_norm)
        try:
